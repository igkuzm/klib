...
static pthread_t reachability(const char *address, int port, int timeout, void *user_data, int (*callback)(void *user_data, bool reachable, char * error));
...
void *check_address_is_reachable(void * params) 
{
	...
	while (1) {
		...
		if (address && *address != 0){
			...
			if (p->callback)
				p->callback(p->user_data, false, (char *)"reachability: cant't read address");
...
pthread_t reachability_dispatch(const char *address, bool find_ip, int port, int timeout, void *user_data, int (*callback)(void * user_data, bool reachable, char * error)){
	...
	if (err){
		if (callback)
			callback(user_data, false, (char *)"reachability: can't set THREAD attributes");
		return 0;
	}

	//allocate Reachability_params
	struct reachability_params * p = (struct reachability_params *)malloc(sizeof(struct reachability_params));
	if (p == NULL) {
		if (callback)
...
pthread_t reachability(const char *address, int port, int timeout, void *user_data, int (*callback)(void *user_data, bool reachable, char * error)){
	if (isalpha(address[0])) {
		return reachability_dispatch(address, true, port, timeout, user_data, callback);
	} else {
		return reachability_dispatch(address, false,  port, timeout, user_data, callback);
	}
}
