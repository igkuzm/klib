...
int k_lib_cp_recursive_callback(char *filename, FILETYPE filetype,
                                void *user_data) {
  struct k_lib_cp_recursive_args *args =
      (struct k_lib_cp_recursive_args *)user_data;
...
int cpr(const char *from, const char *to, bool overwrite) {
  struct k_lib_cp_recursive_args args = { args.source = from,
                                          args.dest = to,
                                          args.depth = 0,
